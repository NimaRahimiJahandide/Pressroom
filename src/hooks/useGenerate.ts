 ...
 *   - Parse SSE events (resume, token, done, error)
 ...
 *
 * Resume-after-cancel:
 *   generate(postId, { continueGeneration: true }) sends { continue: true }
 *   in the POST body. The server responds with a `resume` SSE event carrying
 *   the existing content; the hook calls store.seedTokens() so the editor
 *   shows the full draft before new token events arrive.
 *
...
type GenerateOptions = {
  /** When true, the server seeds the stream with the post's existing content. */
...
type UseGenerateReturn = {
  ...
  generate: (postId: string, options?: GenerateOptions) => Promise<void>;
  cancel: () => Promise<void>;
...
export function useGenerate(): UseGenerateReturn {
  ...
  const generate = useCallback(
    async (postId: string, options?: GenerateOptions) => {
      ...
      try {
        ...
        const response = await fetch('/api/generate', {
          ...
          body: JSON.stringify({
            postId,
            continue: options?.continueGeneration === true,
          }),
        ...
        while (true) {
          ...
          for (const line of lines) {
            if (line.startsWith('event: ')) {
              ...
              if (eventType === 'resume') {
                // Server sent the existing content before any token events.
                // Seed the store so the editor shows the full draft, then
                // token events append on top of it.
                if (typeof data.existingContent === 'string') {
                  useGenerationStore.getState().seedTokens(data.existingContent);
                }
